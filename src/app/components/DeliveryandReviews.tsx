...
/* ─────────────────────────────────────────
   CUSTOMER REVIEWS — types & helpers
───────────────────────────────────────── */
type ApiReview = {
  id: number;
  name: string;
...
// Deterministic avatar palette so the same name always gets the same colors
const AVATAR_PALETTE = [
  { color: "#e8f4f0", textColor: "#1a6b52" },
  { color: "#f0e8f4", textColor: "#6b1a6b" },
  { color: "#f4ede8", textColor: "#6b3a1a" },
  { color: "#e8f0f4", textColor: "#1a4a6b" },
];
...
function toDisplayReview(r: ApiReview): DisplayReview {
  const palette = AVATAR_PALETTE[hashString(r.name) % AVATAR_PALETTE.length];
  const location =
    r.city && r.pincode
      ? `${r.city} - ${r.pincode}`
      : r.city || r.pincode || null;
...
// Shown only while there aren't enough real reviews yet (new business).
// These disappear automatically once MIN_DISPLAY_REVIEWS real reviews exist.
const REAL_REVIEWS_THRESHOLD = 10;
...
const FALLBACK_REVIEWS: DisplayReview[] = [
  {
    ...
    location: "Madurai, 625017",
    quote:
      "Very soft and fresh chapathis. Tastes just like home made. Delivery was on time!",
    rating: 5,
    initials: "RK",
    color: "#e8f4f0",
    textColor: "#1a6b52",
      ...
      >
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          >
            ...
            <Box
              component="span"
              sx={{ fontWeight: 700, color: "var(--primary-maroon-mid)" }}
            >
              Madurai District
            </Box>{" "}
            ,ensuring quality, freshness, and timely doorstep delivery.
        ...
        >
          ...
          >
            Enter your pincode to check if we deliver to your location in
            Madurai District.
          </Typography>
          ...
          <Box
            sx={{
              display: "flex",
              alignItems: "center",
              border: "1.5px solid rgba(90,40,20,0.18)",
              borderRadius: "12px",
              overflow: "hidden",
              background: "#fafaf8",
            }}
          ...
          <Button
            ...
            sx={{
              ...
              textTransform: "none",
              boxShadow: "none",
              "&:hover": { bgcolor: "#2c1009", boxShadow: "none" },
            }}
          >
            Check Availability
          </Button>
            ...
            <Box
              sx={{
                ...
                bgcolor: available
                  ? "rgba(15,110,86,0.08)"
                  : "rgba(180,30,30,0.07)",
                border: `1px solid ${available ? "rgba(15,110,86,0.2)" : "rgba(180,30,30,0.15)"}`,
              }}
            >
              <Box sx={{ flexShrink: 0, mt: 0.1 }}>
                {available ? (
          ...
          {!checked && (
            <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1 }}>
              ...
              >
                We ensure timely and hygienic delivery of ready-to-heat chapathi
                and poori right at your doorstep.
              </Typography>
            </Box>
          )}
        </Box>
...
export function CustomerReviewsSection() {
  ...
  useEffect(() => {
    ...
    fetch("/api/reviews/featured")
      ...
      .then((data: ApiReview[]) => {
        if (cancelled) return;
        const real = Array.isArray(data) ? data.map(toDisplayReview) : [];

        if (real.length >= REAL_REVIEWS_THRESHOLD) {
          // Enough real reviews — static ones are no longer needed
          setReviews(real);
        } else {
          // Not enough real reviews yet — merge static + real, then re-rank
          // together by word count so longer reviews (real or fallback)
          // always surface first, regardless of which list they came from.
          const merged = [...real, ...FALLBACK_REVIEWS].sort((a, b) => {
            const wcA = a.quote.trim().split(/\s+/).filter(Boolean).length;
            const wcB = b.quote.trim().split(/\s+/).filter(Boolean).length;
            return wcB - wcA;
          });
          setReviews(merged);
        }
  ...
  return (
    ...
    >
      ...
      >
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Box
                sx={{
                  ...
                  fontWeight: 700,
                  fontFamily: "var(--font-main)",
                  color: reviews[active].textColor,
                  flexShrink: 0,
                  border: "1.5px solid rgba(90,56,37,0.1)",
                }}
              >
        ...
        {/* Dots below */}
        <Box
          sx={{ display: "flex", justifyContent: "center", gap: 0.8, mt: 1.5 }}
        >
          {reviews.map((_, i) => (
            <Box
              key={i}
      ...
      {/* ── DESKTOP: fixed page of 4 cards, highlight cycles through them ── */}
      {(() => {
        const pageSize = 4;
        const totalPages = Math.ceil(reviews.length / pageSize);
        const currentPage = Math.floor(active / pageSize);
        const pageStart = currentPage * pageSize;
        const highlightOffset = active % pageSize;
        ...
        return (
          <>
            ...
            >
              ...
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#5a3825" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="15 18 9 12 15 6" />
                </svg>
              </Box>

              {/* Cards — fixed set of 4, highlight rotates within them */}
              <Box
                sx={{
                  flex: 1,
                  display: "grid",
              ...
              >
                {[0, 1, 2, 3].map((offset) => {
                  const i = (pageStart + offset) % reviews.length;
                  const r = reviews[i];
                  const isActive = offset === highlightOffset;
                  return (
                    <Box
                      key={i}
                      onClick={() => setActive(i)}
                      ...
                      sx={{
                        ...
                        flexDirection: "column",
                        gap: 1.5,
                        boxShadow: isActive ? "0 8px 28px rgba(122,46,20,0.1)" : "0 2px 8px rgba(0,0,0,0.03)",
                    ...
                    >
                      ...
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1.2 }}>
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column" }}>
                          <Typography sx={{ fontFamily: "var(--font-main)", fontWeight: 700, fontSize: 13.5, color: "var(--primary-maroon-dark)", lineHeight: 1.2 }}>
                            {r.name}
                          </Typography>
                          <Typography sx={{ fontFamily: "var(--font-main)", fontSize: 11.5, color: "#9a6a50", lineHeight: 1.2, mt: 0.2 }}>
                            {r.location}
                          </Typography>
                        </Box>
                      </Box>
            ...
            <Box sx={{ display: { xs: "none", sm: "flex" }, justifyContent: "center", gap: 1 }}>
              {Array.from({ length: totalPages }).map((_, p) => (
                <Box
                  key={p}
                  onClick={() => setActive(p * pageSize)}
                  sx={{
                    width: currentPage === p ? 22 : 8,
                    height: 8,
                    borderRadius: "999px",
                    background: currentPage === p ? "#c25a30" : "rgba(90,56,37,0.18)",
                    cursor: "pointer",
                    transition: "all 0.3s ease",
                  }}
                />
