...
export default function BulkOrderPage() {
  ...
  const pricing = useMemo(() => {
    const rows = productRows.map((r) => {
      ...
      return { ...r, qty: r.qty, qtyNum: qty, unitPrice: UNIT_PRICE, total };
    });
    ...
    const allFilled     = rows.length > 0 && rows.every((r) => r.qtyNum > 0);
  ...
  const handleProductSelect = (e: SelectChangeEvent<string[]>) => {
    ...
    // Sync productRows: keep existing qty if product already in rows
    setProductRows((prev) =>
      val.map((name) => {
        const existing = prev.find((r) => r.name === name);
        const product  = ALL_PRODUCTS.find((p) => p.name === name)!;
        return existing ?? { name, image: product.image, qty: "" };
      })
    );
  };
  ...
  const handleAddProduct = () => {
    // Opens select — just focus it; user picks from dropdown
    const remaining = ALL_PRODUCTS.filter((p) => !selectedNames.includes(p.name));
    if (remaining.length === 0) return;
    const next = remaining[0];
    setSelectedNames((prev) => [...prev, next.name]);
    setProductRows((prev) => [...prev, { name: next.name, image: next.image, qty: "" }]);
  };
...
>
  ...
  {/* LEFT fade — solid bg bleeds into image */}
  <Box
    sx={{
      position: "absolute",
      top: 0,
      ...
      height: "100%",
      background: {
        xs: "linear-gradient(to bottom, #f7f6f3 60%, transparent 100%)",
        md: "linear-gradient(to right, #f7f6f3 45%, rgba(247,246,243,0.85) 65%, rgba(247,246,243,0.3) 82%, transparent 100%)",
      },
      zIndex: 1,
      pointerEvents: "none",
    }}
  ...
  {/* RIGHT fade — fades right after left content ends (~35% from left) */}
  <Box
    sx={{
      position: "absolute",
      top: 0,
      left: { xs: "0%", md: "30%" },   // ← starts where left content ends
      width: { xs: "0%", md: "20%" },  // ← covers the transition zone
      height: "100%",
      background: "linear-gradient(to right, #f7f6f3 0%, rgba(247,246,243,0.5) 40%, transparent 100%)",
      zIndex: 1,
      pointerEvents: "none",
        ...
        <Box sx={{ px: { xs: 3, sm: 4, md: 6 }, py: { xs: 4, md: 5 } }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.75 }}>
            <Typography sx={{
              ...
              textTransform: "uppercase", color: "var(--primary-teal-dark)", whiteSpace: "nowrap",
            }}>
              Fresh. Hygienic. Delivered.
            </Typography>
            <Box sx={{
              width: 26, height: 24, backgroundColor: "var(--green)",
              mask: "url('/plant.png') no-repeat center / contain",
              WebkitMask: "url('/plant.png') no-repeat center / contain", opacity: 0.7,
            }} />
          </Box>
          ...
          <motion.div {...fadeUp(0)}>
            <Typography sx={{
              ...
              fontWeight: 700,
              fontFamily: "var(--font-heading)", color: "var(--primary-maroon-dark)",
              lineHeight: 1.15, mb: 1.25,
            }}>
          ...
          <motion.div {...fadeUp(0.05)}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
              ...
              <Box sx={{
                width: 16, height: 16, backgroundColor: "var(--or)",
                mask: "url('/grain-wheat-icon.svg') no-repeat center / contain",
                WebkitMask: "url('/grain-wheat-icon.svg') no-repeat center / contain", opacity: 0.75,
              }} />
              <Box sx={{ width: 28, height: 2, backgroundColor: "var(--or)", borderRadius: 1 }} />
            </Box>
          </motion.div>
          ...
          <motion.div {...fadeUp(0.1)}>
            <Typography sx={{
              ...
              color: "var(--text)", lineHeight: 1.65, mb: 2.5, maxWidth: 420,
            }}>
              Order fresh Chapathi and Poori in large quantities for weddings,
              corporate events, temple functions, and catering needs.
            </Typography>
          </motion.div>
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: { xs: 2, sm: 2.5 } }}>
              {HERO_BADGES.map((b) => (
                <Box key={b.label} sx={{
                  ...
                  alignItems: "center", gap: 0.5, minWidth: 56,
                }}>
                  <Box sx={{
                    width: 40, height: 40, borderRadius: "50%",
                    backgroundColor: "--org2",
                   
                    borderColor: "color-mix(in srgb, var(--primary-teal-dark), transparent 65%)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    color: "var(--gold)", boxShadow: "0 2px 8px rgba(0,0,0,0.06)",
                  }}>
                    {b.icon}
                  </Box>
                  <Typography sx={{
          ...
          <Grid container spacing={{ xs: 3, md: 4 }} alignItems="flex-start">
            ...
            <Grid item xs={12} md={7}>
              <motion.div {...fadeRight(0.1)}>
                  ...
                  <Grid container spacing={2.5}>
                    ...
                    <Grid item xs={12}>
                      <FormControl fullWidth required sx={fieldSx}>
                        <InputLabel>Products Required</InputLabel>
                        <Select
                          multiple value={selectedNames} onChange={handleProductSelect} label="Products Required"
                          startAdornment={<InputAdornment position="start" sx={{ ml: 1 }}><SearchOutlinedIcon sx={{ color: "var(--primary-teal-dark)", fontSize: 19 }} /></InputAdornment>}
                        ...
                        >
                          {ALL_PRODUCTS.map((p) => (
                            <MenuItem key={p.name} value={p.name}>
                              ...
                              />
                              <ListItemText primary={p.name} primaryTypographyProps={{ fontSize: "0.88rem" }} />
                            </MenuItem>
                          ))}
                        </Select>
                      </FormControl>
                      <Typography sx={{ fontSize: "0.74rem", color: "var(--text)", mt: 0.5, ml: 0.5, opacity: 0.65 }}>
                        {selectedNames.length > 0 ? `${selectedNames.length} product${selectedNames.length > 1 ? "s" : ""} selected` : "You can select multiple products"}
                      </Typography>
                    </Grid>
                    ...
                    {productRows.length > 0 && (
                      <Grid item xs={12}>
                          ...
                          <Table size="small">
                            <TableHead>
                              <TableRow>
                                ...
                                <TableCell sx={tableHeadCellSx}>Quantity (Pieces) *</TableCell>
                                <TableCell sx={tableHeadCellSx} align="right">Unit Price</TableCell>
                                ...
                                <TableRow key={row.name} sx={{ "&:last-child td": { border: 0 } }}>
                                  ...
                                  <TableCell sx={tableCellSx}>
                                    <TextField
                                      ...
                                      value={row.qty} onChange={(e) => handleQtyChange(row.name, e.target.value)}
                                      inputProps={{ min: 1 }}
                                      sx={{
                                        width: 140,
                                        "& .MuiOutlinedInput-root": {
                        ...
                        {/* Add another product */}
                        {selectedNames.length < ALL_PRODUCTS.length && (
                          <Button
                            size="small" startIcon={<AddIcon />} onClick={handleAddProduct}
                            sx={{
                    ...
                    {/* Submit */}
                    <Grid item xs={12}>
                      <Button fullWidth onClick={() => setSubmitted(true)} startIcon={<SendOutlinedIcon />}
                        sx={{
                ...
                {/* Need help strip */}
                <Box sx={{
                  mt: 2, p: "14px 20px", borderRadius: "14px",
                  border: "1.5px solid rgba(0,0,0,0.08)", backgroundColor: "#fff",
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                    ...
                    <Box>
                      <Typography sx={{ fontWeight: 600, fontSize: ".88rem", color: "var(--primary-maroon-dark)", lineHeight: 1.3 }}>
                        Need help with your bulk order?
                      </Typography>
                      <Typography sx={{ fontSize: ".78rem", color: "var(--text)" }}>
                        Our team is ready to assist you with custom requirements.
                      </Typography>
                    </Box>
                  </Box>
                  <Button component={Link} href="/contact" variant="outlined" endIcon={<ArrowRightAltOutlinedIcon />}
                    sx={{
                      ...
                      borderRadius: "10px", borderColor: "var(--primary-teal-dark)",
                      color: "var(--primary-teal-dark)", px: 2.5, py: 0.9, whiteSpace: "nowrap",
              ...
              <motion.div {...fadeLeft(0.2)}>
                <Paper elevation={0} sx={{
                  ...
                  {submitted && pricing.allFilled ? (
                    ...
                    /* Products selected but qty missing */
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, p: "12px 16px", borderRadius: "10px", backgroundColor: "rgba(248,241,230,0.8)", mb: 2.5 }}>
                      ...
                      <Typography sx={{ fontSize: ".83rem", color: "var(--text)" }}>
                        Add quantities to your selected products to see the estimated quote.
                      </Typography>
                    </Box>
                  ) : null}
                  ...
                  {productRows.length > 0 ? (
                    <Box sx={{ borderRadius: "12px", border: "1.5px solid rgba(0,0,0,0.07)", overflow: "hidden", mb: 2 }}>
                            ...
                            <TableRow key={row.name} sx={{ "&:last-child td": { border: 0 } }}>
                              ...
                              <TableCell sx={tableCellSx} align="center">
                                <Typography sx={{ fontSize: "0.83rem", color: "var(--text)" }}>
                                  {row.qtyNum > 0 ? row.qtyNum.toLocaleString("en-IN") : "–"}
                                </Typography>
                              </TableCell>
                              <TableCell sx={tableCellSx} align="right">
                                <Typography sx={{ fontSize: "0.83rem", color: "var(--text)" }}>₹ {row.unitPrice.toFixed(2)}</Typography>
                              </TableCell>
                              <TableCell sx={tableCellSx} align="right">
                      ...
                      <Box sx={{ px: 2, py: 1.5, borderTop: "1.5px solid rgba(0,0,0,0.07)", backgroundColor: "rgba(0,0,0,0.015)" }}>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          ...
                          <Typography sx={{ fontSize: "1.05rem", fontWeight: 800, color: "var(--primary-maroon-dark)" , display :"flex"}}>
                    ...
                    <Box sx={{ textAlign: "center", py: 4, px: 2, mb: 2 }}>
                      ...
                      <Typography sx={{ fontSize: ".8rem", color: "var(--text)", opacity: 0.7 }}>
                        Please fill the form and select products to see the summary.
                      </Typography>
                    </Box>
                  )}

                  {/* Order details (shown after submit) */}
                    ...
                    <Box sx={{ display: "flex", gap: 1.5, mt: 1, flexWrap: "wrap" }}>
                      <Button variant="outlined" startIcon={<FileDownloadOutlinedIcon />}
                        sx={{
                          ...
                          borderRadius: "10px", borderColor: "var(--primary-teal-dark)",
                          color: "var(--primary-teal-dark)", py: 1,
                          "&:hover": { backgroundColor: "color-mix(in srgb, var(--primary-teal-dark), transparent 92%)" },
                        }}
                      >
                        Download Quote (PDF)
                      ...
                      <Button variant="outlined" startIcon={<RefreshOutlinedIcon />} onClick={handleReset}
                        sx={{
                          flex: 1, textTransform: "none", fontWeight: 600, fontSize: ".82rem",
                          borderRadius: "10px", borderColor: "rgba(0,0,0,0.2)",
                          color: "var(--text)", py: 1,
                  ...
                  {/* Why request a quote (only when not submitted) */}
                  {!submitted && (
                    <>
                      <Divider sx={{ my: 2.5 }} />
                      <Box sx={{ borderRadius: "12px", backgroundColor: "rgba(248,241,230,0.65)", p: "16px 18px" }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
                          ...
                          <Typography sx={{ fontWeight: 700, fontSize: ".88rem", color: "var(--primary-teal-dark)" }}>
                            Why request a quote?
                          </Typography>
                        </Box>
                ...
                <Grid item xs={6} sm={4} md={2.4} key={idx}>
                  <motion.div {...fadeUp(idx * 0.08)}>
                    <Box sx={{
                      ...
                      gap: 1.5, p: { xs: 2, md: 2.5 }, borderRadius: "16px",
                      border: "1.5px solid rgba(0,0,0,0.06)", backgroundColor: "#fff", height: "100%",
